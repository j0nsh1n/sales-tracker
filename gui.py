...
class SettingsDialog(tk.Toplevel):
    """The only place orders can be removed."""
    ...
    def __init__(self, master: tk.Tk, tracker: SalesTracker, on_change) -> None:
        ...
        ttk.Label(
            pad,
            text="Orders stay on the list until you reset them here. "
            "Checking someone off only marks them received — it does not delete the row.",
            style="Hint.TLabel",
            wraplength=460,
        ).pack(anchor="w", pady=(6, 16))

        ttk.Label(pad, text="LEDGER FILE", style="Field.TLabel").pack(anchor="w")
        ttk.Label(pad, text=str(tracker.db_path), style="Hint.TLabel", wraplength=460).pack(
            anchor="w", pady=(2, 18)
        )

        ttk.Label(pad, text="RESET", style="Field.TLabel").pack(anchor="w")
        ttk.Label(
            pad,
            text="Type RESET below, then choose what to clear. This cannot be undone.",
            style="Hint.TLabel",
            wraplength=460,
    ...
    def _confirmed(self) -> bool:
        if self.var_confirm.get().strip() != "RESET":
            messagebox.showinfo(
                "Reset",
                "Type RESET in the box first so this cannot happen by accident.",
                parent=self,
            )
            return False
        return True
    ...
    def _reset_orders(self) -> None:
        ...
        self.on_change()
        messagebox.showinfo("Reset", f"Cleared {count} order(s). Products are still on file.", parent=self)
        self.destroy()
